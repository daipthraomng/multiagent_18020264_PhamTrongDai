...
class ReflexAgent(Agent):
    ...
    def evaluationFunction(self, currentGameState, action):
        ...
        return (-2 * nearestfood) + ghost_dist - (100*len(foodList))
...
class MinimaxAgent(MultiAgentSearchAgent):
    ...
    def getAction(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        def minimax_search(state, index, depth):
            # min layer and last ghost
            if index == state.getNumAgents():
                if depth == self.depth:
                    return self.evaluationFunction(state)
                else:
                    return minimax_search(state, 0, depth + 1)
            # not min layer and last ghost
            else:
                moves = state.getLegalActions(index)
                if len(moves) == 0:
                    return self.evaluationFunction(state)
                next = (minimax_search(state.generateSuccessor(index, m), index + 1, depth) for m in moves)
                if index == 0:
                    return max(next)
                else:
                    return min(next)
...
def betterEvaluationFunction(currentGameState):
    ...
    if newCapsules:
        nearestCapsule = min([manhattanDistance(newPos, caps) for caps in newCapsules])
    else:
        nearestCapsule = 0

    if nearestCapsule:
        nearest_capsule = -3 / nearestCapsule
    else:
        nearest_capsule = 100

    if nearestGhost:
        ghost_distance = -2 / nearestGhost
    else:
        ghost_distance = -500
    ...
    return -2 * nearestFood + ghost_distance - 10 * len(foodList) + nearest_capsule
